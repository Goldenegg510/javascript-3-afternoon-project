...
//Code Here
let myStrongest = monstersInYourPocket.filter(function(element, index, monstersInYourPocket){
...
let orderTotals = orders.map(function(element, index, orders){
...
let bobsTotal = purchases.reduce(function(accumulator, curElement, currentIndex, purchases){
  if(curElement["owner"] === "Bob"){
    console.log(curElement["owner"])
    console.log(curElement["price"])
    return  accumulator + curElement["price"]
  } else {
    return accumulator
  }
}, 0)
